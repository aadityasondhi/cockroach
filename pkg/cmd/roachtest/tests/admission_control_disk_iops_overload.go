		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			m.Go(func(ctx context.Context) error {
				...
				for i := 0; i < numIterations; i++ {
					...
					if len(writeBWValues) >= sampleCountForIOPS {
						// TODO(aaditya): We should be asserting on total IOPS once reads
						// are being paced.
						latestSampleMeanForBW := roachtestutil.GetMeanOverLastN(sampleCountForIOPS, writeBWValues)
						if latestSampleMeanForBW > IOPSThreshold {
							t.Fatalf("mean write IOPS over the last 2m %f (last iter: %f) exceeded threshold of %f, read IOPS: %f, total IOPS: %f", latestSampleMeanForBW, writeVal, IOPSThreshold, readVal, totalIOPS)
						}
					}
					numSuccesses++
				}
