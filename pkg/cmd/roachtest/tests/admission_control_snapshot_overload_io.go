		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			c.Run(ctx, option.WithNodes(c.Node(workloadNode)),
				"./cockroach workload init kv --drop --insert-count=40000000 "+
					"--max-block-bytes=65536 --min-block-bytes=65536 {pgurl:1}")

			t.Status(fmt.Sprintf("starting kv workload thread (<%s)", time.Minute))
			m := c.NewMonitor(ctx, c.Range(1, crdbNodes))
			m.Go(func(ctx context.Context) error {
				c.Run(ctx, option.WithNodes(c.Node(workloadNode)),
					fmt.Sprintf("./cockroach workload run kv --tolerate-errors --splits=1000 --histograms=%s/stats.json --read-percent=50 --max-rate=600 --max-block-bytes=65536 --min-block-bytes=65536 --concurrency=1024 {pgurl:1}",
						t.PerfArtifactsDir()))
				return nil
			})
